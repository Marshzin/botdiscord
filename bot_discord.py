...
# Exemplo:
# USUARIOS_AUTORIZADOS = {123456789012345678}
...
async def executar_automacao_no_canal(canal: discord.abc.Messageable):
    global automacao_em_execucao

    if automacao_em_execucao:
        await canal.send("⚠️ Já existe uma automação em execução.")
        return
    ...
    def notify_sync(payload):
        ...
        if tipo == "inicio":
            future = asyncio.run_coroutine_threadsafe(
                canal.send(embed=criar_embed_inicio(payload["total"])),
                loop
            )
            future.result()
            ...
            future = asyncio.run_coroutine_threadsafe(
                canal.send(embed=criar_embed_sucesso(payload["login"])),
                loop
            )
            future.result()
            ...
            future = asyncio.run_coroutine_threadsafe(
                canal.send(embed=criar_embed_sem_atividade(payload["login"])),
                loop
            )
            future.result()
            ...
            future = asyncio.run_coroutine_threadsafe(
                canal.send(embed=criar_embed_erro(payload["login"], payload["mensagem"])),
                loop
            )
            future.result()
            ...
            future = asyncio.run_coroutine_threadsafe(
                canal.send(embed=criar_embed_resumo(payload["resultado"])),
                loop
            )
            future.result()
    ...
    try:
        ...
        await canal.send(embed=criar_embed_erro_geral(e))
    finally:
        automacao_em_execucao = False
...
async def start(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        "✅ Comando recebido. Preparando a automação...",
        ephemeral=True
    )

    asyncio.create_task(executar_automacao_no_canal(interaction.channel))
